...
export class OgService {
  ...
  getOg = async (query: ParsedQs): Promise<Buffer> => {
    const { type, title, image, article, description } =
      validOgQuery.safeParse(query).data ?? {};

    const parsedImage = image ?? this.emiliaImageUrl;
    ...
    const svg = await satori(markup, {
      ...
      height: 600,
      fonts: [
        { name: 'Inter', data: this.interRegular, weight: 400 },
        { name: 'Inter', data: this.interMedium, weight: 500 },
        { name: 'Inter', data: this.interSemibold, weight: 600 }
      ]
    });
...
export async function loadFonts(
  ...
  return Promise.all([
    fetch(`${cloudflareCdnUrl}/assets/inter-regular.ttf`).then((res) =>
      res.arrayBuffer()
    ),
    fetch(`${cloudflareCdnUrl}/assets/inter-medium.ttf`).then((res) =>
      res.arrayBuffer()
    ),
    fetch(`${cloudflareCdnUrl}/assets/inter-semibold.ttf`).then((res) =>
      res.arrayBuffer()
    )
  ]);
}
